...
def _score_model_tag(name: str) -> str:
    ...
    if "NLLB13B" in s or "NLLB_13B" in s or "NLLB-1.3B" in s or "NLLB1.3B" in s:
        return "NLLB13B"
    if "NLLBFT" in s or "NLLB_FT" in s or "NLLB-FT" in s:
        return "NLLBFT"
    if "BAIDU" in s:
        return "BAIDU"
    if "KIMI" in s:
...
def eval_set_nllb(
    ...
    translator_name = str(translator_name or f"NLLB-200({src_lang}->{tgt_lang})")
    print("\n[NLLB Eval 总结]")
...
def ui_main():
    ...
    def v6_browse_nllb_13b_model_dir():
        p = _browse_dir("选择 NLLB 1.3B 模型目录")
        if p:
    ...
    def v6_browse_nllb_ft_model_dir():
        ...
        if p:
            v6["nllb_ft_model_dir"].set(p)

    def v6_browse_nllb_13b_ft_model_dir():
    ...
    def v6_run():
        ...
        def task():
            ...
            if translator_kind == "baidu":
            ...
            elif translator_kind == "nllb_ft":
                if not nllb_ft_model_dir or not os.path.exists(nllb_ft_model_dir):
                    raise RuntimeError("NLLB-FT 模型目录不存在")
                eval_set_nllb(
                    eval_path=eval_path,
                    model_dir=nllb_ft_model_dir,
                    eval_out=None,
                    ...
                    default_category=dataset_kind,
                    translator_name=f"NLLBFT-{dataset_kind}",
                )
            elif translator_kind == "nllb_13b_ft":
                if not nllb_13b_ft_model_dir or not os.path.exists(nllb_13b_ft_model_dir):
                    raise RuntimeError("NLLB-1.3B-FT 模型目录不存在")
                eval_set_nllb(
                    eval_path=eval_path,
                    model_dir=nllb_13b_ft_model_dir,
                    eval_out=None,
                    eval_out_dir=output_dir,
                    eval_sheet=None,
                    n=(n if n > 0 else None),
                ...
                eval_set_nllb(
                    ...
                    batch_size=batch_size,
                    default_category=dataset_kind,
                    translator_name=f"NLLB-{dataset_kind}",
                )
            print("\n[UI] 完成")
    ...
    r += 1
    ttk.Label(tab6, text="Qwen2.5-3B 脚本").grid(row=r, column=0, sticky="w")
    ...
    r += 1
    ttk.Label(tab6, text="NLLB-1.3B 模型目录").grid(row=r, column=0, sticky="w")
