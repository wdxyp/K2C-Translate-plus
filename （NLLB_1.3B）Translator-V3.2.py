...
# ==========================================
# V3.2 (NLLB-200 1.3B 基础模型 / 微调模型双模式)
# ==========================================

NLLB_MODEL_DIR = os.getenv("NLLB_MODEL_DIR") or r"d:\K2C_Translator_Plus\models\models--facebook--nllb-200-1.3B"
...
def run_translation_task(input_file, output_folder):
    ...
    try:
        ...
        output_file = os.path.join(output_folder, (custom_name if custom_name else f"translated_nllb_1.3b_v3.2_{ACTIVE_MODEL_LABEL}_{os.path.basename(input_file).split('.')[0]}") + file_ext)
...
def translation_done_callback(output_file, duration_minutes):
    translate_button.config(state=tk.NORMAL, text="🚀 开始 NLLB 1.3B 翻译")
...
def save_to_corpus(orig, trans):
    if generate_corpus.get() and orig:
        ...
        try:
            ...
            direction = "unknown"
        corpus_file = f'Corpus/Corpus_nllb_1.3b_v3.2_{ACTIVE_MODEL_LABEL}_{direction}_{timestamp}.xlsx'
        to_lang = TO_LANG_MAP.get(direction)
        seen = set()
        filtered_orig = []
        filtered_trans = []
...
# --- UI 布局 ---
root = tk.Tk()
root.title("NLLB 1.3B 翻译工具 V3.2")
...
ttk.Label(main_frame, text="NLLB-200 1.3B 本地翻译 V3.2 (基础模型 / 微调模型)", style="Header.TLabel").pack(pady=(0, 20))
...
translate_button = tk.Button(main_frame, text="🚀 开始 NLLB 1.3B 翻译", command=start_translation, bg="#16a34a", fg="white", font=("微软雅黑", 12, "bold"), relief="flat", cursor="hand2", pady=12)
