...
# ==========================================
# V3.0 (NLLB-200 本地基础模型)
# ==========================================

NLLB_MODEL_DIR = os.getenv("NLLB_MODEL_DIR") or r"d:\K2C_Translator_Plus\models\models--facebook--nllb-200-distilled-600M"
NLLB_DEFAULT_BATCH_SIZE = int(os.getenv("NLLB_BATCH_SIZE", "8"))
NLLB_DEFAULT_MAX_LENGTH = int(os.getenv("NLLB_MAX_LENGTH", "256"))
...
def start_translation():
    global NLLB_MODEL_DIR
    input_file = input_file_entry.get()
    output_folder = output_folder_entry.get()
    model_dir = nllb_model_dir_entry.get().strip()
    if not (input_file and output_folder and model_dir):
        messagebox.showwarning("提示", "请完整选择输入文件、输出目录和 NLLB 模型目录")
        return
    ...
    translate_button.config(state=tk.DISABLED, text="🚀 正在进行 NLLB 翻译，请稍候...")
...
def run_translation_task(input_file, output_folder):
    ...
    try:
        ...
        output_file = os.path.join(output_folder, (custom_name if custom_name else f"translated_nllb_v3.0_{os.path.basename(input_file).split('.')[0]}") + file_ext)
...
def translation_done_callback(output_file, duration_minutes):
    translate_button.config(state=tk.NORMAL, text="🚀 开始 NLLB 翻译")
...
def save_to_corpus(orig, trans):
    if generate_corpus.get() and orig:
        ...
        try:
            ...
            direction = "unknown"
        corpus_file = f'Corpus/Corpus_nllb_v3.0_{direction}_{timestamp}.xlsx'
        to_lang = TO_LANG_MAP.get(direction)
        seen = set()
        filtered_orig = []
        filtered_trans = []
...
ttk.Label(main_frame, text="NLLB-200 本地翻译 V3.0 & 自动校准系统", style="Header.TLabel").pack(pady=(0, 20))
...
ttk.Label(file_card, text="NLLB 模型目录:").grid(row=2, column=0, sticky="w", pady=5)
nllb_model_dir_entry = ttk.Entry(file_card, width=50)
nllb_model_dir_entry.grid(row=2, column=1, padx=10)
nllb_model_dir_entry.insert(0, NLLB_MODEL_DIR)
ttk.Button(file_card, text="选择目录", command=lambda: (nllb_model_dir_entry.delete(0, tk.END), nllb_model_dir_entry.insert(0, filedialog.askdirectory()))).grid(row=2, column=2)
ttk.Label(file_card, text="自定义文件名:").grid(row=3, column=0, sticky="w", pady=5)
custom_filename_entry = ttk.Entry(file_card, width=50)
custom_filename_entry.grid(row=3, column=1, padx=10, columnspan=2, sticky="w")
...
translate_button = tk.Button(main_frame, text="🚀 开始 NLLB 翻译", command=start_translation, bg="#16a34a", fg="white", font=("微软雅黑", 12, "bold"), relief="flat", cursor="hand2", pady=12)
translate_button.pack(fill="x", pady=20)
status_label = ttk.Label(main_frame, text="就绪：已加载校准词典 (revision.md)，等待本地 NLLB 模型翻译。", foreground="#7f8c8d")
status_label.pack()
