...
NLLB_MODEL_DIR = os.getenv("NLLB_MODEL_DIR") or "/content/drive/MyDrive/models/models--facebook--nllb-200-3.3B"
NLLB_BASE_MODEL_DIR = os.getenv("NLLB_BASE_MODEL_DIR") or NLLB_MODEL_DIR
NLLB_FINETUNED_MODEL_DIR = os.getenv("NLLB_FINETUNED_MODEL_DIR") or r""
...
def run_translation_task(input_file, output_folder):
    ...
    try:
        ...
        output_file = os.path.join(output_folder, (custom_name if custom_name else f"translated_nllb_v3.3.1_{ACTIVE_MODEL_LABEL}_{os.path.basename(input_file).split('.')[0]}") + file_ext)
        ...
        translation_done_callback(output_file, duration_minutes)
        return output_file
    except Exception as e:
        err_msg = str(e)
        print(f"[错误] 详情: {err_msg}")
        translation_failed_callback(err_msg)
        raise
    finally:
        _watchdog_stop()
...
def save_to_corpus(orig, trans):
    if generate_corpus.get() and orig:
        ...
        try:
            ...
            direction = "unknown"
        corpus_file = f'Corpus/Corpus_nllb_v3.3.1_{ACTIVE_MODEL_LABEL}_{direction}_{timestamp}.xlsx'
        to_lang = TO_LANG_MAP.get(direction)
        seen = set()
        filtered_orig = []
        filtered_trans = []
...
root = _DummyRoot()
status_label = _DummyWidget()
translate_button = _DummyWidget()
tk = _TkConsts()

model_mode_var = _SimpleVar("base")
translation_direction = _SimpleVar("ko2zh")
append_translation = _SimpleVar(False)
generate_corpus = _SimpleVar(False)
translate_corpus_mode = _SimpleVar(False)

input_file_entry = _DummyEntry()
output_folder_entry = _DummyEntry("/content/drive/MyDrive/outputs")
base_model_dir_entry = _DummyEntry(NLLB_BASE_MODEL_DIR)
finetuned_model_dir_entry = _DummyEntry(NLLB_FINETUNED_MODEL_DIR)
custom_filename_entry = _DummyEntry("")
corpus_file_entry = _DummyEntry("")

DEFAULT_OUTPUT_DIR = "/content/drive/MyDrive/outputs"
DEFAULT_REVISION_FILE = "/content/drive/MyDrive/Files_share(Python)/revision.md"
...
def mount_google_drive():
    try:
        ...
        raise RuntimeError(f"当前环境不是 Google Colab，无法挂载 Drive: {e}")
...
def install_colab_dependencies():
    ...
    import subprocess
    cmds = [
        [sys.executable, '-m', 'pip', 'install', '-q', 'transformers', 'pandas', 'openpyxl', 'python-pptx', 'python-docx', 'tqdm', 'sentencepiece'],
...
def set_revision_file(path):
    global revision_map
    if path and os.path.exists(path):
        revision_map = load_revision_dict(path, silent=True)
        print(f"[系统] 已加载校准词典: {path} ({len(revision_map)} 条)")
    else:
        revision_map = {}
        print(f"[系统] 未加载校准词典，文件不存在: {path}")
...
def configure_colab_runtime(
    ...
    direction="ko2zh",
    append_bilingual=False,
    save_corpus=False,
    custom_filename="",
    output_dir=None,
    revision_file=DEFAULT_REVISION_FILE,
):
    global NLLB_MODEL_DIR, NLLB_BASE_MODEL_DIR, NLLB_FINETUNED_MODEL_DIR, ACTIVE_MODEL_LABEL

    ACTIVE_MODEL_LABEL = (model_mode or "base").strip().lower()
    NLLB_BASE_MODEL_DIR = (base_model_dir or NLLB_BASE_MODEL_DIR or "/content/drive/MyDrive/models/models--facebook--nllb-200-3.3B").strip()
    NLLB_FINETUNED_MODEL_DIR = (finetuned_model_dir or NLLB_FINETUNED_MODEL_DIR or "").strip()
    NLLB_MODEL_DIR = NLLB_BASE_MODEL_DIR if ACTIVE_MODEL_LABEL == "base" else NLLB_FINETUNED_MODEL_DIR

    if not NLLB_MODEL_DIR:
        raise RuntimeError("当前模式未配置模型目录")

    model_mode_var.set(ACTIVE_MODEL_LABEL)
    translation_direction.set(direction)
    append_translation.set(bool(append_bilingual))
    generate_corpus.set(bool(save_corpus))
    custom_filename_entry.set(custom_filename or "")
    output_folder_entry.set(output_dir or DEFAULT_OUTPUT_DIR)
    base_model_dir_entry.set(NLLB_BASE_MODEL_DIR)
    finetuned_model_dir_entry.set(NLLB_FINETUNED_MODEL_DIR)

    set_revision_file(revision_file)
    os.makedirs(output_folder_entry.get(), exist_ok=True)

    print(f"[系统] 当前模式: {ACTIVE_MODEL_LABEL}")
    print(f"[系统] 当前翻译方向: {translation_direction.get()}")
    print(f"[系统] 当前模型目录: {NLLB_MODEL_DIR}")
    print(f"[系统] 输出目录: {output_folder_entry.get()}")


def translate_file_in_colab(
    input_file,
    output_dir=None,
    direction="ko2zh",
    model_mode="base",
    ...
    if not input_file or not os.path.exists(input_file):
        raise FileNotFoundError(f"待翻译文件不存在: {input_file}")

    configure_colab_runtime(
        model_mode=model_mode,
        base_model_dir=base_model_dir,
        finetuned_model_dir=finetuned_model_dir,
        direction=direction,
        append_bilingual=append_bilingual,
        save_corpus=save_corpus,
        custom_filename=custom_filename,
        output_dir=output_dir or os.path.dirname(input_file) or DEFAULT_OUTPUT_DIR,
        revision_file=revision_file,
    )

    input_file_entry.set(input_file)
    if not check_direction_mismatch(input_file):
        raise RuntimeError("翻译方向预检未通过，已取消执行")
    return run_translation_task(input_file, output_folder_entry.get())


def translate_corpus_in_colab(
    corpus_file,
    model_mode="base",
    base_model_dir=None,
    finetuned_model_dir=None,
    ...
    configure_colab_runtime(
        ...
        append_bilingual=False,
        save_corpus=False,
        output_dir=os.path.dirname(corpus_file) or DEFAULT_OUTPUT_DIR,
        revision_file=revision_file,
    )
    corpus_file_entry.set(corpus_file)
    return translate_corpus_file()


def quick_translate(input_file, output_dir=None):
    return translate_file_in_colab(input_file=input_file, output_dir=output_dir)


if __name__ == "__main__":
    print("=" * 68)
    print("NLLB-200 V3.3.1 (in-colab) - Google Colab 运行版")
    print("=" * 68)
    print("示例:")
    print("1. mount_google_drive()")
    print("2. configure_colab_runtime(model_mode='base')")
    print("3. translate_file_in_colab('/content/drive/MyDrive/input.docx')")
    print("4. translate_corpus_in_colab('/content/drive/MyDrive/corpus.xlsx')")
